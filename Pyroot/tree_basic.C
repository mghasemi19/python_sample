...
void gaus_tree(){
   ...
   TH1F *h = new TH1F("h", "h", 100, -5, 5); h->FillRandom("gaus", 1000);
...
void loop_tree(){
  ...
  float pt, pt_sig;
  tree->SetBranchAddress("ptsumf", &pt);
  sig_tree->SetBranchAddress("ptsumf", &pt_sig);
  
  for (int n=0; n<tree->GetEntries();n++){
    tree->GetEntry(n);
    sig_tree->GetEntry(n);
    if (pt > pt_sig) std::cout << "Bkg_pT:" << pt << std::endl;
    if (pt == pt_sig) std::cout << "Bkg_pT = Sig_pT:" << pt << std::endl;
    if (pt < pt_sig) std::cout << "Sig_pT:" << pt_sig << std::endl;
  }
}
...
void loop_Iter(){
   ...
   while ((key=(TKey*)next())) {
       ...
       //key->Print();
       tree->Draw("ptsumf>>htest", "", "goff"); std::cout << htest->GetMean();
...
void Merge_Tree(){
  ...
  TFile *ofile = new TFile("/Users/meisamghasemi/Desktop/Code-factory/Root/root-6.18.04/tutorials/mlp/total.root", "RECREATE");
  TList *list = new TList;
  list->Add(bkg_tree);
  list->Add(sig_tree);
  TTree *new_tree = TTree::MergeTrees(list);
  new_tree->SetName("tot_foltered"); new_tree->Print();
...
void update_tree(){
  TFile *infile = new TFile("/Users/meisamghasemi/Desktop/Code-factory/Root/root-6.18.04/tutorials/mlp/mlpHiggs.root", "update");
  TTree *bkg_tree = (TTree*) infile->Get("bg_filtered");
  float test, pt, m;
...
void removeTree(){
  ...
  std::string file_name="/Users/meisamghasemi/Desktop/Code-factory/Root/root-6.18.04/tutorials/mlp/mlpHiggs.root";
  TFile *file=new TFile((file_name).c_str(),"update");
  std::string object_to_remove="sig_filtered;4";
  std::string object_to_remove2="sig_filtered;2";
